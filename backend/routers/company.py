...
def create_company(company : CompanyCreate,db:Session=Depends(get_db,),current_user=Depends(rol_required(["admin"]))):
...
def update_company(company_id: int,company: CompanyUpdate,db:Session=Depends(get_db),current_user=Depends(rol_required(["admin"]))):
...
def delete_company(company_id: int,db:Session=Depends(get_db),current_user=Depends(rol_required(["admin"]))):
    db_company = db.query(Company).filter(Company.id == company_id).first()
    if not db_company:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Company not found")
